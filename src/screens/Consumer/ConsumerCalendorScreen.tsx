...
const StatusLegend: React.FC<{ isDistributor: boolean }> = React.memo(({ isDistributor }) => (
  <View style={calendarScreenStyles.legendContainer}>
    ...
    <View style={calendarScreenStyles.legendGrid}>
      ...
      {/* Always show extra milk in legend when viewing consumer calendar */}
      <View style={calendarScreenStyles.legendItem}>
        <View style={[calendarScreenStyles.legendDot, { backgroundColor: statusColors.extra_milk }]} />
        <Text style={calendarScreenStyles.legendText}>Extra Milk</Text>
      </View>
...
const ConsumerCalendarScreen: React.FC<CalendarViewerProps> = ({
  ...
  // FIXED: Always show extra milk for the consumer whose calendar is being viewed
  const milkRequestsForCustomer = useMemo(() => {
    if (!customerId || !upcomingMilkRequests) { return []; }
    const value = upcomingMilkRequests[customerId];
    return Array.isArray(value) ? value : [];
  }, [upcomingMilkRequests, customerId]); // Removed isDistributor check

  // Updated markedDates - now includes extra milk even when viewed by distributor
  const markedDates: MarkedDates = useMemo(() => {
    const marks: MarkedDates = {};

    // When distributor views consumer calendar, show all statuses including extra_milk
    const allowedStatuses: string[] = ['delivered', 'vendor_unavailable', 'distributor_unavailable', 'cancelled', 'leave', 'extra_milk', 'customer_paused', 'missed', 'not_requested'];
    ...
    // 2. Customer leaves
    if (Array.isArray(leavesForCustomer) && leavesForCustomer.length > 0) {
      leavesForCustomer.forEach((leave: LeaveItem) => {
    ...
    // 3. Extra milk requests - NOW ALWAYS INCLUDED for the consumer whose calendar is shown
    if (Array.isArray(milkRequestsForCustomer) && milkRequestsForCustomer.length > 0) {
      console.log('Processing extra milk requests:', milkRequestsForCustomer.length);
      milkRequestsForCustomer.forEach((request: ExtraMilkItem) => {
        ...
        if (date) {
          ...
          if (!allStatusesPerDate[date].includes('extra_milk')) {
            ...
            console.log('Added extra milk dot for date:', date);
          }
        }
      });
    }
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {/* Only show action buttons and lists for actual consumer, not distributor viewing */}
        {!isDistributor && (
          <>
            {leavesForCustomer.length > 0 && (
              <View style={calendarScreenStyles.leavesContainer}>
