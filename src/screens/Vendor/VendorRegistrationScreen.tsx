...
interface VendorPayload {
  ...
  jarshi_milk_litre?: number;
  buffalo_milk_litre?: number;
  cr?: number;  // Cow Milk Rate key as per backend
  br?: number;  // Buffalo Milk Rate key as per backend
}
...
export default function VendorRegisterScreen({ navigation }: { navigation: any }) {
  ...
  const validate = () => {
    ...
    if (hasCow) {
      ...
      if (!cowMilkRate.trim() || isNaN(Number(cowMilkRate)) || Number(cowMilkRate) <= 0) {
        return 'Please enter a valid Cow milk rate in rupees';
      }
    }
    ...
    if (hasBuffalo) {
      ...
      if (!buffaloMilkRate.trim() || isNaN(Number(buffaloMilkRate)) || Number(buffaloMilkRate) <= 0) {
        return 'Please enter a valid Buffalo milk rate in rupees';
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Assign rates separately and always
      vendorPayload.cr = Number(cowMilkRate);
      vendorPayload.br = Number(buffaloMilkRate);
  ...
  return (
    ...
    >
      ...
      {/* Milk Rates fields (always visible) */}
      <View style={styles.formGroup}>
