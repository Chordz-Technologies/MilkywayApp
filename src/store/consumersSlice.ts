...
// // store/slices/consumersSlice.ts
...
// // Enhanced delivery record with distributor tracking
...
//   // ✅ DAILY RESET FIELDS (these get reset every day)
//   deliveryStatus?: 'delivered' | 'cancelled' | 'pending';
//   lastDeliveryDate?: string;
//   lastDeliveryRemarks?: string;

//   // ✅ PERMANENT FIELDS (these stay forever)
...
//   lastActiveDate: string | null; // ✅ Track last active date for daily reset
// }
...
// // ✅ DAILY RESET: Check and perform daily reset
...
//     // If it's a new day, trigger selective reset
//     if (lastActiveDate && lastActiveDate !== today) {
//       console.log('🗓️ New day detected! Resetting delivery fields only...');
//       console.log(`Last: ${lastActiveDate}, Today: ${today}`);
...
//     try {
//       console.log('🔍 Redux: Fetching assigned consumers for milkman ID:', milkmanId);
//       const response = await getDistributorAssignedConsumers(milkmanId);
//       console.log('✅ Redux: Assigned consumers API response:', response.data);
...
//     } catch (error: any) {
//       console.error('❌ Redux: Error fetching assigned consumers:', error);
...
//     try {
//       console.log('🚚 Redux: Marking delivery:', payload);
//       const response = await markDeliveryAsSuccessful(payload);
//       console.log('✅ Redux: Delivery marked successfully:', response.data);
...
//       };
//     } catch (error: any) {
//       console.error('❌ Redux: Error marking delivery:', error);
...
//     // ✅ SELECTIVE DAILY RESET: Only reset delivery status fields
//     performDailyDeliveryReset: (state) => {
//       console.log('🔄 Performing selective daily reset for delivery fields...');

//       // Reset ONLY delivery status fields, keep everything else
//       state.consumers.forEach(consumer => {
//         // ✅ RESET ONLY THESE FIELDS (today's delivery status):
...
//         // - milk_requirement, vendor_name, assignment_date
//         // - deliveryHistory (historical data preserved)
//         // - assigned_distributor_id, status, id
//         // - All other consumer data remains untouched
//       });

//       // Update last active date
//       state.lastActiveDate = new Date().toISOString().split('T')[0];

//       console.log('✅ Selective daily reset completed - only delivery fields reset');
//     },
...
//     builder
//       // Fetch Assigned Consumers with Smart Merge
...
//         // ✅ SMART MERGE: Preserve local delivery data when refreshing
//         if (state.consumers.length > 0) {
//           // Merge logic: keep local delivery status, update other data from API
//           const mergedConsumers = action.payload.map((apiConsumer: any) => {
//             // Find existing consumer with same ID
//             const existingConsumer = state.consumers.find(
//               c => c.customer_id === apiConsumer.customer_id
//             );

//             if (existingConsumer) {
//               // ✅ SELECTIVE MERGE: Keep local delivery fields, update everything else from API
//               return {
//                 ...apiConsumer, // API data (name, contact, address, milk_requirement, etc.)
//                 // ✅ PRESERVE LOCAL DELIVERY FIELDS:
...
//       // Mark Delivery - Enhanced with Daily Reset Integration
...
//           // ✅ UPDATE DAILY DELIVERY FIELDS (will be reset tomorrow)
...
//           // ✅ UPDATE PERMANENT DELIVERY HISTORY
